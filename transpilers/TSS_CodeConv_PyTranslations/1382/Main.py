...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        osr = OutputStreamWriter(System.out)
        o = PrintWriter(osr)
        fr = FastReader()
        n = fr.nextDouble()
        m = fr.nextDouble()
        i = 0
        while i < m - (math.fmod(n, m)):
            o.print(str(int((n / m))) + " ")
            i += 1
        for i in range(int((m - (math.fmod(n, m)))), m):
            o.print(str(int(ceil(n / m))) + " ")
        o.close()
class FastReader:
    def __init__(self):
        #instance fields found by Java to Python Converter:
...
#JAVA TO PYTHON CONVERTER WARNING: Method 'throws' clauses are not available in Python:
#ORIGINAL LINE: public int [] intArray(int size) throws IOException
    def intArray(self, size):
        ...
        return arr
#JAVA TO PYTHON CONVERTER WARNING: Method 'throws' clauses are not available in Python:
#ORIGINAL LINE: public char [] charArray() throws IOException
