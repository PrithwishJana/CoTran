class Main:
    ...
    Main.IN = Scanner(System.in)
    re = BufferedReader(InputStreamReader(System.in))
    wr = BufferedWriter(OutputStreamWriter(System.out))
    @staticmethod
...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        ...
        while T > 0:
            ...
            for i in range(0, n):
                arr [i][0] = Main.IN.nextInt()
                arr [i][1] = i + 1
            Arrays.sort(arr, lambda o1, o2 : o1 [0] - o2 [0])
            if n >= 2:
                print(str(arr [0][1]) + " " + str(arr [n - 1][1]))
            else:
                print(str(1) + " " + str(1))
            T -= 1
        out.flush()
