...
class Doctor:
    def __init__(self, num):
        ...
        self.decimals = (java.math.BigDecimal(num - self.integers)).setScale(4, java.math.BigDecimal.ROUND_UP).doubleValue()
    def judge(self):
        if self.integers > 255 or self.decimals > 0.9375:
            return False
        return True
    def _toBinaryDigits(self):
        tmp = ""
        count = 1
        i = integers
        while i > 0:
            tmp += math.fmod(i, 2)
            i = math.trunc(i / float(2))
        sb = StringBuffer(tmp)
        i = 0
