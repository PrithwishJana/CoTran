...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        ...
        while True:
            ...
            if Main.n == 0:
                return
            Main.cost = [[0 for _ in range(Main.n + 1)] for _ in range(Main.n + 1)]
            ans = 0
            i = 0
            while i < Main.n - 1:
                a = in_.nextInt()
            ...
            while i <= Main.n:
                ...
                j = 1
                while j <= Main.n:
                    if Main.cost [i][j] > 0 or Main.cost [i][j] == - 1:
                        reaf += 1
                        ii = i
                        jj = j
                    j += 1
                if i != 1 and reaf == 1:
                    ans -= Main.cost [ii][jj] * 2
                    Main.cost [ii][jj] = Main.cost [jj][ii] = - 1
                i += 1
            print(ans - Main.max(1))
    @staticmethod
    def max(m):
