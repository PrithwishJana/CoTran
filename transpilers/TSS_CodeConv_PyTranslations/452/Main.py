...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        ...
        n = scanner.nextInt()
        q = scanner.nextInt()
        Main.parents = [0 for _ in range(n)]
        Main.weight = [0 for _ in range(n)]
        Main.rank = [0 for _ in range(n)]
        for i in range(0, n):
            Main.parents [i] = i
        type = 0
        x = 0
...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def find(x):
        if x == Main.parents [x]:
            ...
            r = Main.find(Main.parents [x])
            Main.weight [x] += Main.weight [Main.parents [x]]
            return Main.parents [x] = r
    @staticmethod
    def weight(x):
        Main.find(x)
        return Main.weight [x]
