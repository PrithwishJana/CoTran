        ...
        def isPossible(self, width):
            ...
            while i < self.n:
                ...
                if w > width:
                    ...
                    w = self.books [i]
                i += 1
            return False if cnt > self.m else True
        def doIt(self):
            while True:
                self.m = Main.IN.nextInt()
                self.n = Main.IN.nextInt()
                if self.n + self.m == 0:
                    return
                self.books = [0 for _ in range(self.n)]
                i = 0
                while i < self.n:
                    self.books [i] = Main.IN.nextInt()
                    i += 1
                left = 1
