...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def _run(self):
        ...
        tokenizer = java.util.StringTokenizer(reader.readLine())
        N = int(tokenizer.nextToken())
        R = int(tokenizer.nextToken())
        L = int(tokenizer.nextToken())
        ...
        for i in range(0, R):
            tokenizer = java.util.StringTokenizer(reader.readLine())
            d = int(tokenizer.nextToken()) - 1
            t = int(tokenizer.nextToken())
            x = int(tokenizer.nextToken())
            c [index] += t - now
            po [d] += x
            now = t
            if x > 0:
