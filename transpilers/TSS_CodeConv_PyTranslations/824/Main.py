...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def solve(self):
        ...
        while Main.w != 0:
            Main.h = sc.nextInt()
            Main.grid = [['\0' for _ in range(Main.w)] for _ in range(Main.h)]
            i = 0
            while i < Main.h:
                line = sc.next()
                Main.grid [i] = line.toCharArray()
                i += 1
            self.countB = 0
            self.countW = 0
            i = 0
            while i < Main.h:
                ...
                while j < Main.w:
                    if Main.grid [i][j] == '.':
                        ...
                        self.countGrid = 0
                        Main._dfs(j, i)
                        if Main.B and not self.W:
                            self.countB += self.countGrid
                        if (not Main.B) and self.W:
                            self.countW += self.countGrid
                    j += 1
                i += 1
            print(str(self.countB) + " " + str(self.countW))
            Main.w = sc.nextInt()
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def dfs(self, x, y):
        Main.grid [y][x] = 'x'
        ...
        for i in range(0, 4):
            nx = x + self.dx [i]
            ny = y + self.dy [i]
            if nx < Main.w and nx >= 0 and ny < Main.h and ny >= 0:
                if Main.grid [ny][nx] == 'B':
