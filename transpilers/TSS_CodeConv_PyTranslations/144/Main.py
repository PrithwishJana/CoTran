...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        ...
        while True:
            ...
            for i in range(0, n):
                additions = java.util.TreeSet()
                bJust = Main._calcNearestWeight(a [i], w, 0, 0, additions)
                if not bJust:
            ...
            if commonAdditions is None:
                ...
                if commonAdditions.size() > 0:
                    print(commonAdditions.first())
                else:
                    print(- 1)
        scan.close()
    @staticmethod
    def _calcNearestWeight(x, w, i, sum, additions):
        if x == sum:
            return True
        if i == len(w):
            additions.add(abs(x - sum))
            return False
