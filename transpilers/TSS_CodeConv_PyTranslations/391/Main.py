...
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        ...
        while True:
            ...
            i = 0
            while i < len(field):
                minTime = min(minTime, Main.crossTime(i - 20, field [i], r))
                minTime = min(minTime, Main.crossTime(i + 1 - 20, field [i], r))
                i += 1
            print(minTime)
    @staticmethod
    def crossTime(x, y, r):
        d = int((- 2 * (y + r))) ** 2 - 4 * (x * x + y * y + 2 * y * r)
        if d <= 0:
            return Main.INF
        else:
