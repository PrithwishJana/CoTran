...
#ORIGINAL LINE: public static void main(String [] args) throws java.text.ParseException
#JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def main(args):
        t1 = Main.input.next()
        t2 = Main.input.next()
        date = java.text.SimpleDateFormat("yyyy:MM:dd")
...
#JAVA TO PYTHON CONVERTER TASK: Java to Python Converter cannot determine whether both operands of this division are integer types - if they are then you should change 'lhs / rhs' to 'math.trunc(lhs / float(rhs))':
        print(abs(date.parse(t1).getTime() - date.parse(t2).getTime()) / (1000 * 60 * 60 * 24))
