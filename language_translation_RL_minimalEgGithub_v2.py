...
def getDataset(path_dict, tokenizer, multigpuFLAG, srcLang = "python"):
    ...
    if not multigpuFLAG:
        ...
        tokenized_train = [(tokenize_code_maxLenPadding(sample[0], tokenizer).flatten(),
                            sample[0],
...
if __name__ == '__main__':
    ...
    datasetDict = getDataset(pathDict[args.src_lang + "2" + args.dest_lang + "_debug"], 
                                        fwdTokenizer, args.multigpu) # + "_debug"
    #print (datasetDict)
    ...
    if args.multigpu:
        train_dataloader = DataLoader(datasetDict["train"], batch_size = args.train_batch_size)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in tqdm(training_dataloader):
            ...
            if args.multigpu:
                for i in range(len(src_id_list)):
                    src_id_list[i] = src_id_list[i][src_id_list[i] != fwdTokenizer.pad_token_id]
