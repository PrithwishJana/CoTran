...
def getDataset(path_dict, tokenizer, srcLang = "python"):
    ...
    tokenized_train = []
    for sample in training_dataset[:10000]: #NOTE: [:10000] dataset reduced
...
def RLTrain(model, tokenizer, datasetDict, writeDir, dest_lang):
    ...
    fwdPPO_config = PPOConfig(
        ...
        batch_size = 512, log_with="wandb") #, batch_size = 256,, log_with="wandb" NOTE!!!!!!!!!!!!!!
    ...
    for epoch in range(20):
        ...
        if epoch % 1 == 0:
            test(tester_dataloader, fwdModel, fwd_test_kwargs, fwdTokenizer, str(epoch)+"RL", 
                        device, dest_lang)
...
if __name__ == '__main__':
    ...
    #--------------- TRAINING LOOP ----------------
    for ep in range(1):
        model, device = RLTrain(model, fwdTokenizer, datasetDict, args.writeDir, args.dest_lang)
