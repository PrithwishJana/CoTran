...
def getDataset(path_dict, tokenizer):
    ...
    tokenized_train = [(tokenize_code(sample[0], tokenizer).flatten(),
                        tokenize_code(sample[1], tokenizer).flatten(),
                         0) for 
                    sample in training_dataset] #NOTE: put 0 instead of  sample[2], to reduce memory
...
def test(dataloader, model, kwargs, tokenizer, epoch, device):
    ...
    for test_batch in tqdm(dataloader):
        ...
        test_pred_idList = model.generate(src_id_mat.to(device), 
                                            attention_mask = attention_mask_mat.to(device), 
                                            **kwargs)
...
if __name__ == '__main__':
    ...
    ppo_config = PPOConfig(
        ...
        batch_size = args.train_batch_size) #, log_with="wandb" NOTE!!!!!!!!!!!!!!
    ...
    test(tester_dataloader, fwdModel, generation_kwargs, fwdTokenizer, "init", device)
    #f.write(f"{line}\n")
    ...
    for epoch in range(args.num_epochs):
        ...
        #testing
        test(tester_dataloader, fwdModel, generation_kwargs, fwdTokenizer, epoch, device)
